...
// PR #2 ships an in-process logger-based stub; future PRs will plug in
// Telegram / email / PagerDuty by replacing the implementation.
//
// Channels we want eventually:
//   - Telegram bot (lowest-latency human-readable channel)
//   - Email (auditable, slower)
//   - On-call paging (escalation only)
//
...
type Alerter interface {
	...
	XenditError(ctx context.Context, orderNo, action string, err error)
...
func (a *LogAlerter) XenditError(ctx context.Context, orderNo, action string, err error) {
	logger.L(ctx).Error("ALERT: Xendit call failed",
		zap.String("alert", "xendit_error"),
		zap.String("order_no", orderNo),
